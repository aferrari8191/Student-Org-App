function Model() {
    ...
    this.generateNsid = function(_params) {
        ...
        if (Ti.App.Properties.hasProperty("FLICKR_NSID") && Ti.App.Properties.hasProperty("FLICKR_USERNAME") && Ti.App.Properties.getString("FLICKR_USERNAME") == _params.username) {
            _params.callback();
            return;
        }
        Ti.App.Properties.setString("FLICKR_USERNAME", _params.username);
        _params.username.indexOf("@") > 0 ? this.handleNsid({
            user: {
                id: _params.username
            }
        }, null, _params.callback) : HTTP.request({
            timeout: 1e4,
            type: "GET",
            format: "JSON",
    ...
    this.handleSets = function(_data, _url, _callback) {
        ...
        if (_data.photosets.photoset.length > 0) {
            ...
            for (var i = 0, x = _data.photosets.photoset.length; x > i; i++) {
    ...
    this.retrieveSet = function(_params) {
        ...
        UTIL.isStale(ApiBase + "photosets.getPhotos&extras=url_sq,url_m&privacy_filter=1&media=photos&photoset_id=" + _params.id), 
        _params.cache ? HTTP.request({
            timeout: 1e4,
            type: "GET",
            format: "JSON",
    ...
    this.handleSet = function(_data, _url, _callback) {
        ...
        if (_data.photoset.photo.length > 0) {
            var db = Ti.Database.open("ChariTi");
            db.execute("BEGIN TRANSACTION;");
            for (var i = 0, x = _data.photoset.photo.length; x > i; i++) {
    ...
    this.getPhoto = function(_id, _index, _setid) {
        ...
        var data = null;
        var temp = null;
        _id ? data = db.execute("SELECT * FROM flickr_photos_" + TID + " WHERE id = " + UTIL.cleanEscapeString(_id) + " LIMIT 1;") : _index && (data = db.execute("SELECT * FROM flickr_photos_" + TID + " WHERE indx = " + UTIL.cleanEscapeString(_index) + " AND set_id = " + UTIL.cleanEscapeString(_setid) + " LIMIT 1;"));
        while (data.isValidRow()) {
            _id ? temp = {
                id: data.fieldByName("id"),
                set_id: data.fieldByName("set_id"),
